...
impl XdfWriter {
    ...
    pub fn write_stream_footer(&self, stream_id: u32, xml_content: &str) -> anyhow::Result<()> {
        let mut w = self.inner.lock().unwrap();
        write_chunk(&mut *w, ChunkTag::StreamFooter, Some(stream_id), xml_content.as_bytes())?;
        w.flush()?;
        Ok(())
    }
