...
fn main() -> Result<()> {
    ...
    let input = &args[1];
    let format = args.iter().position(|a| a == "--to")
...
#[derive(Debug)]
enum InputType { Xdf, Parquet, Unknown }

fn input_type(path: &str) -> InputType {
    if path.ends_with(".xdf") { InputType::Xdf }
    else if path.ends_with(".parquet") || std::path::Path::new(path).is_dir() { InputType::Parquet }
    ...
    while pos < data.len() {
        ...
        match tag {
            2 => { // StreamHeader
                ...
                let xml = std::str::from_utf8(&data[chunk_start+6..chunk_start+length as usize]).unwrap_or("");
                let name = extract_xml_tag(xml, "name").unwrap_or_default();
                let nch: usize = extract_xml_tag(xml, "channel_count").and_then(|s| s.parse().ok()).unwrap_or(1);
                let srate: f64 = extract_xml_tag(xml, "nominal_srate").and_then(|s| s.parse().ok()).unwrap_or(0.0);
                streams.insert(sid, XdfStream { stream_id: sid, name, nch, srate, timestamps: vec![], data: vec![] });
            }
            3 => { // Samples
                let sid = u32::from_le_bytes(data[chunk_start+2..chunk_start+6].try_into()?);
...
fn parse_samples_f32(content: &[u8], stream: &mut XdfStream) {
    ...
    if p >= content.len() { return; }
    let nlb = content[p] as usize; p += 1;
    if p + nlb > content.len() { return; }
    ...
    let interval = if stream.srate > 0.0 { 1.0 / stream.srate } else { 0.0 };

    for _ in 0..n_samples {
        if p >= content.len() { break; }
        // Timestamp
        let ts_len = content[p] as usize; p += 1;
...
fn xdf_info(path: &str) -> Result<()> {
    ...
    for s in &streams {
        ...
        eprintln!("  stream_id={}: name=\"{}\", {}ch, {}Hz, {} samples, ts {}",
...
fn xdf_to_parquet(input: &str, output_dir: &str) -> Result<()> {
    ...
    for s in &streams {
        ...
        let path = format!("{}/stream_{}_{}.parquet", output_dir, s.stream_id,
            s.name.chars().map(|c| if c.is_alphanumeric() { c } else { '_' }).collect::<String>());
...
fn xdf_to_csv(input: &str, output: &str) -> Result<()> {
    ...
    for s in &streams {
        ...
        for ch in 0..s.nch { write!(file, ",ch{}", ch)?; }
        writeln!(file)?;
